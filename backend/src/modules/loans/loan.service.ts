...
async borrow(teacherId: string, dto: BorrowLoanDto) {
  ...
  return this.repo.prisma.$transaction(async (tx) => {
    ...
    if (!sameSchool) throw new AppError("Student cannot borrow this book (different school)", 403);

    const sameClassroom = book.classroom.id === student.classroom.id;
    if (!sameClassroom) throw new AppError("Student cannot borrow this book (same school, but different class)", 403);
    ...
    const created = await this.repo.createLoan(
      { teacherId, bookId: book.id, studentId: student.id, dueAt },
      tx
    );
