...
export class LoanRepository {
  ...
  findBookByQrForTeacher(teacherId: string, qrToken: string, tx: any = prisma) {
  ...
  findStudentForTeacher(teacherId: string, studentId: string, tx: any = prisma) {
  ...
  createLoan(params: { teacherId: string; bookId: string; studentId: string; dueAt?: Date | null }, tx: any = prisma) {
  ...
  closeLoan(params: { loanId: string; status: "RETURNED" | "LATE"; returnedAt: Date }, tx: any = prisma) {
    return tx.loan.update({
      where: { id: params.loanId },
      data: { status: params.status, returnedAt: params.returnedAt },
      include: {
        book: {
          select: {
            id: true, title: true, qrToken: true, universe: true, publisher: true,
  ...
  setBookStatus(bookId: string, status: "AVAILABLE" | "LOANED" | "RESERVED", tx: any = prisma) {
    return tx.book.update({ where: { id: bookId }, data: { status } });
  }
